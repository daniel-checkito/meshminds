// Vercel serverless function — replaces the n8n idea-validator workflow.
// No npm dependencies: uses native fetch (Node 18+) for Jina AI scraping + Anthropic API.

// ─── Ported from n8n "Extract Product Context" node ───────────────────────────
function extractProductContext(markdown, title, description, sourceURL, imageUrl) {
  let text = markdown;
  ...
  let ctx = `URL: ${sourceURL}\nTITLE: ${title}\nDESCRIPTION: ${description}\n`;
  if (stats.releaseDate) ctx += `RELEASE DATE: ${stats.releaseDate}\n`;
  if (stats.printTime)   ctx += `PRINT TIME: ${stats.printTime}\n`;
  if (stats.plates)      ctx += `PLATES: ${stats.plates}\n`;
  if (stats.layerHeight) ctx += `LAYER HEIGHT: ${stats.layerHeight}\n`;
  if (stats.infill)      ctx += `INFILL: ${stats.infill}\n`;
  if (stats.walls)       ctx += `WALLS: ${stats.walls}\n`;
  if (stats.license)     ctx += `LICENSE: ${stats.license}\n`;
  ctx += `\nPAGE CONTENT (cleaned):\n${text.substring(0, 2500)}`;

  return { productContext: ctx, imageUrl, sourceUrl: sourceURL };
}

// ─── Ported from n8n "Parse Claude Response" node ─────────────────────────────
...
function parseClaudeResponse(text) {
  ...
  function tryParse(s) {
    ...
    for (let i = 0; i < s.length; i++) {
      ...
      if (escaped) {
        const valid = '"\\/bfnrtu';
        sanitized += valid.includes(c) ? '\\' + c : c;
        escaped = false;
      } else if (c === '\\' && inString) {
        escaped = true;
      } else if (c === '"') {
        sanitized += c; inString = !inString;
      } else if (inString && c === '\n') {
        sanitized += '\\n';
...
// ─── Prompt builder ────────────────────────────────────────────────────────────
function buildPrompt(productContext, imageUrl, sourceUrl, body) {
  const arr = (v) => Array.isArray(v) ? v.join(', ') : (v || 'none');

  return `You are an expert Etsy seller, 3D printing business analyst, and product compliance specialist. Your job is to evaluate whether a 3D printed model is worth selling — and whether 3D printing is even the right manufacturing method.
...
- Target marketplace: ${body.marketplace || 'Etsy'}
- Price range: ${body.priceRange || 'not specified'}
- Print time per unit: ${body.printTime || 'not specified'}
- Material: ${body.material || 'not specified'}
- Seller goal: ${body.sellerGoal || 'not specified'}
- Number of printers: ${body.printerCount || 'not specified'}
- Printer type: ${body.printerType || 'not specified'}
- Seller experience level: ${body.experience || 'not specified'}
- Weekly time available: ${body.weeklyTime || 'not specified'}
- Already selling on: ${arr(body.activeMarkets)}
- Social media channels: ${arr(body.socialMedia)}
- Selling from (country): ${body.sellFrom || 'not specified'}
- Selling to (markets): ${body.sellTo || 'WORLDWIDE'}
- Legal / IP flags noted by seller: ${arr(body.legalFlags)}
...
module.exports = async function handler(req, res) {
  ...
  // ── Scrape with Jina AI (free, no key required) ──────────────────────────────
  let productContext = `URL: ${inputUrl}\nTITLE: (not scraped)\nDESCRIPTION: (not scraped)\n`;
  let imageUrl = '';
  ...
  if (inputUrl.startsWith('http')) {
    try {
      ...
      const headers = { Accept: 'application/json', 'X-Timeout': '15' };
      ...
      if (jinaResp.ok) {
        ...
        const d = jinaData.data || {};
        const markdown = d.content || d.text || '';
        const firstImage = d.images ? Object.values(d.images)[0] : '';
        const extracted = extractProductContext(
          markdown,
          d.title || '',
          d.description || '',
          d.url || inputUrl,
        ...
        console.error('Jina scrape HTTP', jinaResp.status);
      }
    } catch (e) {
      console.error('Jina scrape error:', e.message);
      // Proceed without scraped context — Claude will do its best
    }
  }
  ...
  try {
    claudeResp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 4096,
        messages: [{ role: 'user', content: buildPrompt(productContext, imageUrl, sourceUrl, body) }],
      }),
      signal: AbortSignal.timeout(55000),
    });
  } catch (e) {
    console.error('Claude fetch error:', e.message);
    return res.status(502).json({ error: 'analysis_failed', message: 'Claude request timed out' });
  }

  if (!claudeResp.ok) {
    const errText = await claudeResp.text().catch(() => '');
    console.error('Claude API error', claudeResp.status, errText);
    let message = `Claude API error ${claudeResp.status}`;
    try { const j = JSON.parse(errText); message = j.error?.message || message; } catch (_) {}
    return res.status(502).json({ error: 'analysis_failed', message });
  }

  const claudeData = await claudeResp.json();
  const text = (claudeData.content?.[0]?.text || '').trim();

  if (!text) return res.status(502).json({ error: 'analysis_failed', message: 'Empty response from Claude' });
  ...
  try {
    ...
    return res.status(502).json({ error: 'analysis_failed', message: 'Could not parse AI response' });
  }
};
